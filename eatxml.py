...
'''
This notebook is meant to develop and test eatxml.
Eatxml (Elsevier Api To XML) reads information from Elsevier Search API for
UF-Authored articles and for each, it seeks it in the Elsevier Full-text API.
If not found, it logs an error message, otherwise it outputs a file named
pii_{pii}.xml in the given output directory for each article.
...
def get_elsevier_api_uf_query_url_by_dates(d_run_params, verbosity):
    '''
    Caller provides an integer year to search Elsevier for UF-affiliated articles.
    The result data is returned.
    '''
    cymd_low = d_run_params.get('cymd-start')
    cymd_hi = d_run_params.get('cymd-end','')
...
def result_entries_collect(d_params, results_tree, d_batch, verbosity=1, scopus_uf_check=1):
    ...
    '''
    Given d_params dictionary and an article search results tree, loop over the
    tree's entries, where each entr is for a result article,
    and we save each entry in file entry_{pii}.xml,
    and we query the 'full-text' xml api from Elsevier.

    We write an file named pii_{pii_norm_text}.xml in the output dir for each entry/article.

    Note: we also could add a dictionary argument used to track incoming pii values
    nd reject duplicate pii values across calls here during processing the results of a query,
    ...
    for i,entry in enumerate(results_tree.findall('.//{*}entry')):
        ...
        if verbosity > 0:
            print ("\n***** Entry={}, pii={}, pii_norm={}".format(n_entry, pii.text, pii_norm_text))
        if scopus_uf_check:
        ...
        out_filename = '{}/entry/entry_{}.xml'.format(out_base_dir, pii_norm_text)
        with open(out_filename, 'wb') as outfile:
            outfile.write(entry_xml_str)

        # print("ENTRY_XML_STR={}".format(xml_str))
        full_filename = '{}/full/pii_{}.xml'.format(out_base_dir, pii_norm_text)
        for link in entry.findall('{*}link'):
            ...
            # Use this entry's self-link value to parse pii and compose and issue a full-text API textxml retrieval,
...
'''
Method eatxml: Elsevier Api To XML - Read the Elsevier Search API for UF-Authored
...
Params cymd_lo and hi are publication date ranges for the search API use to select
which articles for which to return metadata.
'''
def eatxml(d_params, verbosity=0):
    #d_search_pii = OrderedDict()
    output_articles = 0
    url_initial_search = get_elsevier_api_uf_query_url_by_dates(d_params, verbosity)

    search_query = d_params.get('api-search-query','')
    if verbosity > 0:
        print ('***** INITIAL QUERY PARAM = "{}"\n'.format(search_query))
        print ('***** INITIAL API_SEARCH_URL = "{}"\n'.format(url_initial_search))
    entries_collected = 0
    entries_excepted = 0
    ...
    total_results = results_tree.find('.//opensearch:totalResults', namespaces=d_ns).text
    d_params.update({
        'total-result-entries': "0" if not total_results else str(total_results),
        })
    print("\n***** TOTAL RESULT ENTRIES = {} *****".format(total_results))
...
agent_creator_host = 'UFLIBSHS3TC42'
agent_creator_software = "eatxml" #may add attributes programming-language, revision-date, hash ...
...
out_base_dir = 'c:/users/lib-adm-podengo/downloads/elsevier/output_eatxml/'
...
if (1 == 1): # test with or without call to eatxml
    entries_collected, entries_excepted = eatxml(d_params, verbosity=verbosity)
...
d_params.update({
    "entries-collected" : entries_collected,
    "entries-excepted" : entries_excepted,
    "secsz-end" : secsz_end,
...
headers_txt = '''Server: api.elsevier.com  9999
X-ELS-Status: REQUESTOR_NOT_ENTITLED(The requestor is not entitled to access the resource specified.)
X-ELS-ReqId: 000001555a4d8fa4-4766
X-ELS-APIKey: d91051fb976425e3b5f00750cbd33d8b
X-ELS-TransId: c63b528e-4c52-4790-b1ac-6bc2dea80529
Allow: HEAD,GET
X-ELS-BodyText: false
Content-Type: text/xml;charset=UTF-8
Content-Length: 194
Date: Thu, 16 Jun 2016 17:51:21 GMT
Connection: close
X-RE-Ref: 1 1466099481531187
