...
def sequence_entitlements(
    sequence_rows=None, max_updates=None,batch_size=100, verbosity=0):
    ...
    for  row in sequence_rows:
        index_pii += 1
        if index_pii is not None and index_pii > max_updates:
            print("{}: piis exceeded max={}. Breaking".format(me,max_updates))
            break
        pii = row['pii']
        if verbosity > 0:
        ...
        if index_pii % batch_size == 0:
            ...
            sep = ''
            # Do always print this
            if 1==1 or verbosity > 0:
                print("{}: Batch number={},sending url='{}'"
                     .format(me, batch_number, url))
                sys.stdout.flush()
...
'''
def xxsequence_piis(update_table, verbosity=''):
    stmt = select()
    return
# end sequence_piis()
...
'''
def  xxcreate_elsevier_entitlement_uf(env=None):
    if env == 'uf':
...
def run_elsevier_entitlement_updates(
    env='uf', batch_size=None, max_updates=None,verbosity=0):
    ...
    if verbosity > 0:
        seq_rows = sequence_rows_by_table(
            engine=engine, table=update_table, verbosity=0);
        print("{}: verbosely showing some seq_rows".format(me))
        row_index = 0
        for seq_row in seq_rows:
            row_index += 1
            if row_index > max_updates:
                print("{}: pii index > {}. Breaking".format(me,max_updates))
                break;
            print("{}:sequence item {} seq_row={}"
                 .format(me,row_index,seq_row))
        # end sample loop
    ...
    seq_entitlements = sequence_entitlements(
      sequence_rows=seq_rows, max_updates=max_updates,
    ...
    for row_entitlement in seq_entitlements:
        ...
        for colname, value in row_entitlement.items():
            if colname == 'pii' and verbosity > 0:
...
#end run_elsevier_entitlement_updates()
